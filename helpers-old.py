...
def getProducts():
    try:
        ...
        return (True, "Load products successful", rows)
    except:
        return (False, "Could not load products! Please try again later.", None)
...
def getFirstName(id):
    try: 
        rows = db.execute("SELECT first_name FROM users WHERE user_id=?", id)
        if len(rows) == 0:
            return (False, "Could not find user!", None)
        return (True, "User loaded successfully!", rows[0]['first_name'])
    except:
        return (False, "Could not load user data - please try again later!", None)
